...
function makeBundle(name, entry, out, mode) {
    ...
    return {
        ...
        externals: [
            ({ request }, callback) => {
                // CesiumJS is loaded from CDN → window.Cesium
                if (request === "cesium" || request.startsWith("cesium/")) {
                    return callback(null, "Cesium");
                }
                callback();
            },
            ({ request }, callback) => {
...
module.exports = (_env, argv) => {
    ...
    return [
        // ------------------------------------------------------------------
        // mcp-cesium  — CesiumJS adapters + shared behaviors.
        // Consumer: any CesiumJS scene that needs the MCP camera/light/mesh
        // adapters.  @dev/behaviors is bundled in (not externalized).
        // ------------------------------------------------------------------
        makeBundle("McpCesium", "src/index.ts", "mcp-cesium.js", mode),
    ];
};
